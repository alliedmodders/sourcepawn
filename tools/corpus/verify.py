...
# Tool for interacting with a .smx corpus to find behavorial differences between
# compiler versions.
def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('verifier', type = str, help = 'Path to verifier')
    ...
    files = []
    get_all_files(args.corpus, ['.smx'], files)
...
class Runner(object):
    ...
    def compile(self, path):
        if path.endswith('.smx'):
            ...
            output = None
            try:
                subprocess.check_output(argv, stderr = subprocess.STDOUT, timeout = 10, env = env)
                ok = True
