...
void
PerfJitFile::Write(void* address, uint64_t length, const char* symbol) {
    if (!file_) {
        return;
    }

    fprintf(file_, "%" KE_FMT_X64 " %" KE_FMT_X64 " %s\n", (uint64_t)address, length, symbol);
...
void
PerfJitdumpFile::Write(void* address, uint64_t length, const char* symbol,
