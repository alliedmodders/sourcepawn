...
CodePool::AllocateFor(size_t askBytes) {
    ...
    size_t bytes = (askBytes < kMinPoolSize) ? kMinPoolSize : ke::Align(askBytes, kPageGranularity);
    assert(ke::IsAligned(bytes, kPageGranularity));

#if defined(_WIN32)
    void* address =
        (uint8_t*)VirtualAlloc(nullptr, bytes, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if (!address)
        return nullptr;
#else
    void* address =
        mmap(nullptr, bytes, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_ANON, -1, 0);
