...
SmxV1Image::validate() {
    ...
    for (const char* iter = header_strings_ + last_header_string;
         iter < reinterpret_cast<const char*>(buffer() + hdr_->dataoffs); iter++) {
        if (*iter == '\0') {
    ...
    if (debug_symbols_section_) {
        ...
        if (const Section* globals = findSection(".dbg.globals")) {
            ...
            if (Environment::get()->IsDebugBreakEnabled() &&
                !validateDebugVariables(rtti_dbg_globals_))
                return false;
        }
        if (const Section* locals = findSection(".dbg.locals")) {
            ...
            if (Environment::get()->IsDebugBreakEnabled() &&
                !validateDebugVariables(rtti_dbg_locals_))
                return false;
        }
...
SmxV1Image::LookupFunction(uint32_t code_offset) const {
    ...
    if (debug_syms_unpacked_) {
        auto name = lookupFunction<sp_u_fdbg_symbol_t, sp_u_fdbg_arraydim_t>(debug_syms_unpacked_,
                                                                             code_offset);
        if (name)
            return name;
    }
...
SmxV1Image::NumFunctions() const {
    ...
    if (num_debug_functions == 0) {
        if (debug_syms_)
            num_debug_functions =
                getFunctionCount<sp_fdbg_symbol_t, sp_fdbg_arraydim_t>(debug_syms_);
        else
            num_debug_functions =
                getFunctionCount<sp_u_fdbg_symbol_t, sp_u_fdbg_arraydim_t>(debug_syms_unpacked_);
    }
...
SmxV1Image::GetFunctionName(size_t index, const char** filename) const {
    ...
    if (debug_syms_) {
        ...
        return getFunctionName<sp_u_fdbg_symbol_t, sp_u_fdbg_arraydim_t>(debug_syms_unpacked_,
                                                                         filename, index);
    }
}
...
SmxV1Image::getFunctionAddress(const SymbolType* syms, const char* function, ucell_t* funcaddr,
    ...
    for (uint32_t i = index; i < debug_info_->num_syms; i++) {
        ...
        const SymbolType* sym = reinterpret_cast<const SymbolType*>(cursor);
        if (sym->ident == sp::IDENT_FUNCTION && sym->name < debug_names_section_->size &&
