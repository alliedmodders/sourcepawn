...
static int
cip_map_entry_sort_cmp(const void* a1, const void* a2) {
...
static int
cip_map_entry_cmp(const void* a1, const void* aEntry) {
...
CompiledFunction::FindCipByPc(void* pc) {
    ...
    if (!cip_map_sorted_) {
        qsort(cip_map_->buffer(), cip_map_->size(), sizeof(CipMapEntry), cip_map_entry_sort_cmp);
