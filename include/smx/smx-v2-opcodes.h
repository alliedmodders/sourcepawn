...
#define OPCODE_LIST_V2(FOR_EACH) \
    ...
    FOR_EACH(INITARRAY, 65, "initarray", 21) \
    FOR_EACH(HEAP_SAVE, 66, "heap.save", 1) \
    FOR_EACH(HEAP_RESTORE, 67, "heap.restore", 1) \
    FOR_EACH(TEST_F32, 68, "test.f32", 1) \
    FOR_EACH(NEG_F32, 69, "neg.f32", 1) \
    FOR_EACH(MUL_F32, 70, "mul.f32", 1) \
    FOR_EACH(DIV_F32, 71, "div.f32", 1) \
    FOR_EACH(ADD_F32, 72, "add.f32", 1) \
    FOR_EACH(SUB_F32, 73, "sub.f32", 1) \
    FOR_EACH(EQ_F32, 74, "eq.f32", 1) \
    FOR_EACH(NEQ_F32, 75, "neq.f32", 1) \
    FOR_EACH(LESS_F32, 76, "less.f32", 1) \
    FOR_EACH(LEQ_F32, 77, "leq.f32", 1) \
    FOR_EACH(GRTR_F32, 78, "grtr.f32", 1) \
    FOR_EACH(GEQ_F32, 79, "geq.f32", 1) \
    FOR_EACH(CVT_F32, 80, "cvt.f32", 1) \
    FOR_EACH(MOD_F32, 81, "mod.f32", 1) \
    FOR_EACH(CVT_I64, 83, "cvt.i64", 1) \
    FOR_EACH(TRUNCATE_I64, 84, "truncate.i64", 1) \
    FOR_EACH(TEST_I64, 85, "test.i64", 1) \
    FOR_EACH(INVERT_I64, 86, "invert.i64", 1) \
    FOR_EACH(NEG_I64, 87, "neg.i64", 1) \
    FOR_EACH(SMUL_I64, 88, "smul.i64", 1) \
    FOR_EACH(SDIV_I64, 89, "sdiv.i64", 1) \
    FOR_EACH(ADD_I64, 90, "add.i64", 1) \
    FOR_EACH(SUB_I64, 91, "sub.i64", 1) \
    FOR_EACH(SHL_I64, 92, "shl.i64", 1) \
    FOR_EACH(SSHR_I64, 93, "sshr.i64", 1) \
    FOR_EACH(SHR_I64, 94, "shr.i64", 1) \
    FOR_EACH(EQ_I64, 95, "eq.i64", 1) \
    FOR_EACH(NEQ_I64, 96, "neq.i64", 1) \
    FOR_EACH(OR_I64, 97, "or.i64", 1) \
    FOR_EACH(AND_I64, 98, "and.i64", 1) \
    FOR_EACH(XOR_I64, 99, "xor.i64", 1) \
    FOR_EACH(STOR_S_C_I64, 100, "stor.s.c.i64", 11) \
    FOR_EACH(SLESS_I64, 101, "sless.i64", 1) \
    FOR_EACH(SLEQ_I64, 102, "sleq.i64", 1) \
    FOR_EACH(SGRTR_I64, 103, "sgrtr.i64", 1) \
    FOR_EACH(SGEQ_I64, 104, "sgeq.i64", 1) \
    FOR_EACH(SMOD_I64, 105, "smod.i64", 1) \
    FOR_EACH(STOR_S_I64, 106, "stor.s.i64", 3) \
    FOR_EACH(SWAP, 107, "swap", 1) \
    FOR_EACH(LOAD_FN, 108, "load.fn", 5) \
    FOR_EACH(LOAD_S_I64, 109, "load.s.i64", 3) \
    FOR_EACH(STOR_GLB_I64, 110, "stor.glb.i64", 5) \
    FOR_EACH(LOAD_GLB_I64, 111, "load.glb.i64", 5) \
    FOR_EACH(LOAD_I_I64, 112, "load.i.i64", 1) \
