...
void CompoundEmitter::AddInlineEnumStruct(EnumStructDecl* es, ArrayExpr* array) {
    ...
    if (emitted < static_cast<size_t>(es->array_size()))
        pending_zeroes_ += static_cast<size_t>(es->array_size()) - emitted;

    assert(data_size() - start_pos == static_cast<size_t>(es->array_size()));
}
