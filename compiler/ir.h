...
class Value : public Node {
  public:
    Value(IrKind kind, Expr* expr, QualType type)
      : Node(kind, expr),
        type_(type)
    {}
...
class Array final : public Value {
  public:
    Array(Expr* expr, QualType type, const std::vector<Value*>& values)
      : Value(IrKind::Array, expr, type)
    {
    ...
    static bool is_a(Node* op) { return op->kind() == IrKind::Array; }
...
class ArrayInitializer final : public Value {
  public:
    ArrayInitializer(ArrayExpr* expr, QualType type, std::vector<ir::Value*>&& values)
      : Value(IrKind::ArrayInitializer, expr, type),
        values_(std::move(values))
    {}

    ArrayExpr* expr() const { return pn_u.array_expr; }
...
class IncDecWithTemp final : public IncDec {
  public:
    IncDecWithTemp(IncDecExpr* expr, QualType type, Lvalue* lval, Value* val, uint32_t temp_slot,
                   bool used)
      : IncDec(IrKind::IncDecWithTemp, expr, type, lval, used),
        val_(val),
        slot_(temp_slot)
    {}
    ...
    static bool is_a(Node* op) { return op->kind() == IrKind::IncDecWithTemp; }
