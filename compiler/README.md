...
In addition, the following phases are implemented:
 ...
 - `NameResolver` attaches itself as a delegate to the Parser. The parser sends it notifications, and it uses these to build the scope hierarchy, symbol table, and perform all name binding. It also tries to resolve types if it can, since this alleviates pressure on the TypeResolver pass which is much more expensive.
