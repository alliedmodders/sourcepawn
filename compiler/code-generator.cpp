...
void CodeGenerator::EmitIncDec(ir::IncDec* incdec) {
    ...
    if (save_pri)
        __ emit(OP_PUSH_PRI);
    EmitLoadEpilogue(lval);
    // We might need to initialize a temporary slot with the pre-incremented
    // value, if the increment operator was overloaded.
    if (auto with_temp = incdec->as<ir::IncDecWithTemp>())
        __ emit(OP_STOR_S_PRI, GetSlotAddr(with_temp->slot()));
    if (save_pri)
        __ emit(OP_POP_ALT);
    if (postfix)
        __ emit(OP_PUSH_PRI);
...
void CodeGenerator::EmitCallOp(ir::CallOp* call) {
    ...
    assert(!target->fun()->decl()->return_array());
    ...
    if (auto bound = target->as<ir::BoundFunction>()) {
        EmitValue(bound->val());
        __ emit(OP_PUSH_PRI);
        nargs++;
    }
